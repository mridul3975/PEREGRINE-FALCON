    ...
    try {
        ...
        const decisionResult = await generateText({
            ...
            system: `You are an intelligent job agent. Your task is to analyze a given job summary against the user's resume.
                    ...
                    After any tool calls are complete, your final output MUST be a JSON object strictly conforming to the provided schema.`,
            messages: [
                {
                    role: 'system',
        ...
        const decision = jobDecisionSchema.parse(JSON.parse(decisionResult.text));
